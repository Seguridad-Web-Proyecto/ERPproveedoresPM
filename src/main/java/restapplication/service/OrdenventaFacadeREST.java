...
public class OrdenventaFacadeREST extends AbstractFacade<Ordenventa>
{
    ...
    public Response agregarDetalles(Ordenventa venta)
    {
        try
        {
            ...
            if (venta.getVentadetalleCollection() == null)
            {
                return Response.status(Status.BAD_REQUEST).build();
            }
        ...
        try
        {
            ...
            try
            {
                Ordenventa pedidoGenerado
                        = APIConsumerProveedor.generarPedidoCompleto("Pedido para de proveedor a subproveedor", detalles);
            } catch (Exception ex)
            {
    ...
    {
        List<Ordenventa> ordenes = super.findRange(new int[]
        {
            from, to
        });
        List<Ordenventa> returnList = new ArrayList<>();
